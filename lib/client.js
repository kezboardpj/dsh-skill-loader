 ...
 * `/skill-select <csv>` command; the host persists the selection in its
 * settings document (`skill-loader` namespace), and the picker keeps a
 * localStorage mirror for display on reopen (dsh's settings API does not
 * expose third-party namespaces to the browser, so the mirror is the only
 * sanctioned read path; the host remains authoritative for enforcement). No
 * stored selection = dsh default = every skill loaded, so the initial state
 * is "all checked".
 */
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ---------------------------------------------------------------------
    // Local mirror of the host-stored selection (the host persists it in the
    // dsh settings document; this browser-side mirror is what the picker
    // displays on reopen — see the header notes).
    // ---------------------------------------------------------------------
    const MIRROR_PREFIX = 'dsh-skill-loader:selection:'
    function readMirror(sessionId) {
      try {
        const raw = localStorage.getItem(MIRROR_PREFIX + sessionId)
        if (raw === null) return null
        const parsed = JSON.parse(raw)
        return Array.isArray(parsed) ? parsed : null
      } catch { return null }
    }
    function writeMirror(sessionId, names) {
      try { localStorage.setItem(MIRROR_PREFIX + sessionId, JSON.stringify(names)) } catch { /* non-fatal */ }
    }

    // ---------------------------------------------------------------------
    // The picker chip + panel (rendered through a react-dom root).
    // ---------------------------------------------------------------------
    function SkillPickerChip({ sessionId, t, api, runCommand }) {
      const [skills, setSkills] = react.useState(null) // null = loading
      const [selection, setSelection] = react.useState(() => readMirror(sessionId)) // names | null = default all
      ...
      // Reload the mirror when the chip is reused for another session.
      react.useEffect(() => {
        setSelection(readMirror(sessionId))
    ...
    const inject = ['locale', 'connection', 'remote', 'remote.commands', 'sessions']
    ...
    function apply(ctx) {
      ...
      const sync = () => {
        ...
        root.render(jsx.jsx(SkillPickerChip, { sessionId: current, t, api, runCommand }))
      }
