 ...
 * `/skill-select <csv>` command; the current selection is derived from the
 * session's durable log (latest `skill-loader/selection` event). No selection
 * event = dsh default = every skill loaded, so the initial state is
 * "all checked".
 */
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function SkillPickerChip({ sessionId, t, api, runCommand }) {
      ...
      const [selection, setSelection] = react.useState(null) // null = default all (optimistic)
      ...
      const refresh = react.useCallback(async () => {
        ...
        try {
          ...
          const fromLog = selectionFromEvents(await fetchHistory(api, sessionId))
          setSelection(fromLog)
          lastSentRef.current = fromLog === null ? null : fromLog.join(',')
        } catch {
      ...
      }, [api, runCommand, sessionId, skills, t, refresh])
