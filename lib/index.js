 ...
 *
 * Mechanics:
 * - `/skill-select <csv>` appends a durable log-only `skill-loader/selection`
 *   event (whole-value) carrying the selected skill names.
 ...
 *   names.
 * - Sessions with no selection event keep dsh's default behavior untouched.
 *
 * Deliberately import-free (only node:crypto): a `link:`-installed checkout
 * resolves this package's imports from its own directory, so importing
 * in-box @deepseek-ai/* packages here would break boot.
 */
import { randomUUID } from 'node:crypto'

export const name = 'skill-loader'

// All three registries are mounted by the shipped web profile.
export const inject = ['commands', 'skills', 'tools']

const SELECTION_EVENT = 'skill-loader/selection'
...
export function apply(ctx) {
  ctx.commands.register({
    ...
    recordInput: false,
    handler: (invocation) => handleSelect(ctx, invocation),
  })
  ...
  ctx.on('tools/pre-execute', (exec, next) => {
    ...
    const selection = currentSelection(agent)
...
// ---------------------------------------------------------------------------
// Selection state (durable, log-only, whole-value).
// ---------------------------------------------------------------------------

/** Current selection for one agent, or null when the session never selected anything. */
function currentSelection(agent) {
  for (let index = agent.session.events.length - 1; index >= 0; index -= 1) {
    const event = agent.session.events[index]
    if (event.type !== SELECTION_EVENT) continue
...
async function handleSelect(ctx, invocation) {
  ...
  agent.session.append(SELECTION_EVENT, { names })
